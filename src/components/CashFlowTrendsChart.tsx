...
export const CashFlowTrendsChart: React.FC<CashFlowTrendsProps> = ({
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="rounded-xl bg-slate-900 border border-slate-700 p-3 shadow-xl text-xs space-y-1.5 z-50">
  ...
  return (
    <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 text-slate-100 space-y-4">
      {/* Header & Chart Toggle */}
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 pb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Tren Arus Kas Masuk & Keluar RT.03</h3>
      ...
      <div className="grid grid-cols-3 gap-2 text-center text-xs">
        <div className="p-2.5 rounded-xl bg-slate-950/80 border border-slate-800">
          ...
          <p className="font-mono font-bold text-emerald-400 text-xs mt-0.5 truncate">
        ...
        <div className="p-2.5 rounded-xl bg-slate-950/80 border border-slate-800">
          ...
          <p className="font-mono font-bold text-rose-400 text-xs mt-0.5 truncate">
            {formatRupiah(totalKeluar4Bulan)}
          </p>
        </div>

        <div className="p-2.5 rounded-xl bg-slate-950/80 border border-slate-800">
          <span className="text-[10px] text-slate-400 block">Surplus Bersih</span>
          <p className="font-mono font-bold text-teal-300 text-xs mt-0.5 truncate">
          ...
          {chartType === 'bar' ? (
            <BarChart data={monthlyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Legend
                ...
                iconType="circle"
              />
              <Bar dataKey="pemasukan" name="Kas Masuk" fill="#10b981" radius={[4, 4, 0, 0]} />
              <Bar dataKey="pengeluaran" name="Kas Keluar" fill="#f43f5e" radius={[4, 4, 0, 0]} />
            </BarChart>
          ) : (
